...
def lightWebScraper(link):
    ...
    try:
        name = html_content.find("h1", class_ = "pdp-mfe-hx12kt").text #Gets the HTLM line that gets the product name
        if name == None:
            raise
    ...
    try:
        price = html_content.find("span", class_ =  "pdp-pricing--highlight pdp-pricing__selected pdp-mfe-oan334").text #Checks first tag
        if price is None:
            raise
    except Exception as e:
        try:
            price = html_content.find("span", class_ =  "pdp-pricing__selected pdp-mfe-oan334").text#Checks alternate
            if price is None:
                raise   
        except:
